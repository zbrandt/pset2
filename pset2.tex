...
\begin{homeworkProblem}
    ...
    In contrast to our original regression, our point estimate from the second stage is a weighted average of the treatment effect $\beta_{1i}$, where the weights increase with the degree to which the instrument $D_i$ influences $T_i$ in the first stage:
    \[
        \begin{split}
...
\begin{homeworkProblem}
    ...
    where
        \begin{itemize}[label={}, itemsep=0pt]
            ...
            % \item $Y_i$ is the \textit{dependent variable}, annual firm sales 
            % \item $D_i$ is the \textit{dummy variable}, $D_i=1$ if the firm is a local Chamber of Commerce member and $0$ otherwise 
            \item $\widehat{\beta_0}$ is the \textit{intercept} of the regression. This sample mean value of the logarithm of local average monthly household incomes is 6.485 when average years of education is 0. This result is statistically significant at the 0.01 level.
            \item $\widehat{\beta_1}$ is the coefficient on $educ\_years_i$, associating a one year increase in the average years of education with an 18.3\% increase in local average monthly household incomes. This result is statistically significant at the 0.01 level.
    ...
    One reason that there might be reverse causality bias at play is because wealthier people (measured in terms of local average monthly household incomes) might already puruse more education than those who don't. Thus there may be a positive bias from our dependent variable now explaining some of the variation in the explanatory variable.
    ...
    where
        \begin{itemize}[label={}, itemsep=0pt]
            \item $\widehat{\beta_0}$ is the \textit{intercept} of the regression. This sample mean value of the average years of education is 9.738 when the distance to the US border is 0. This result is statistically significant at the 0.01 level.
            \item $\widehat{\beta_1}$ is the coefficient on $ln(dist\_us\_km)$, associating a one percent increase in the distance to the US border with 0.004 decrease in the average years of education. This result is statistically significant at the 0.01 level.
    ...
    It's possible that our instrument also does not pass the instrument exclusion restriction. While the effect of the logarithm of distance to the US border might not be statistically significant, its effect on the logarithm of local average monthly household incomes is. In this case, our instrument does not have an affect on the dependent variable only through our endogenous explanatory variable but rather through other ways.
        ...
        \] 
    \\
    For a potential instrumental variable to be a valid candidate, the effect it has on the dependent variable must only act through the explanatory one (through `a' and then `b' in the diagram above). If it impacts $Y$ through `c' for instance, it is not valid, and from our regression table above, this seems to be the case considering the point estimate is non-zero and statistically significant at the 0.01 level. However, we would need to include more instruments for the average years of education in our first-stage regression to perform an over-identification test to see if one of the instruments is exogenous and therefore invalid.
