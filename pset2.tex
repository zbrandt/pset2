...
\begin{homeworkProblem}
    ...
    where
        \begin{itemize}[label={}, itemsep=0pt]
            ...
            \item $\beta_0$ is the \textit{intercept} of the regression, the mean value of the logarithm of annual, non-member firm sales 
            \item $\beta_1$ is the coefficient on $D_i$, associating a change in $D_i$ from 0, representing non-membership in a local Chamber of Commerce, to 1, membership, with a $100 \times \beta_1\%$ change in $Y_i$
    ...
    An ordinary least squares regression will help the Ministry understand the causal effect of being a member in a local Chamber of Commerce by producing a point estimate for $\beta_1$ when applied to predict the dependent variable. In the model above, a dummy variable distinguishes member and non-member firms and an error term represents all other factors beyond membership in a local Chamber of Commerce that may explain variation in annual firm sales.
    ...
    \\
    The average treatment effect on the treated (ATT) is then the same thing as the LATE: the causal effect of being a member of a local Chamber of Commerce estimated on the group of firms that chose to take up the experiment when offered to them. This is different from the ATE because it was not randomized which firms will actually follow / adhere to their assigned treatment status. It's instead an independent decision made by each firm. In a rational world, the ATT will usually demonstrate a stronger effect than the ATE because the firms that take up the treatment probably have more to gain than those who don't.
...
\begin{homeworkProblem}
    ...
    \\
    \textit{Municipality code.} The municipality code mean and standard deviation have no practical meaning. The codes range from 1001 to 32056. Each code uniquely identifies a municipality. \\
    \\
    \textit{} Average monthly incomes have a low mean value of 2538.081 and a relatively large standard devation of 1257.101. The distribution could be right skewed as the mean is far closer to the minimum value  of 299.757 than the maximum value of 14,792.910. \\
    \\
    \textit{Proportion of the indigenous population.} The mean value for the indigenous proportion of Mexican municipalities population stands at 11.4\%. However, the standard devation is very high (0.221), suggesting a broad distribution of proportions from 0.000 to a max of 0.984. Some municipalities have effectively 0\% and some are near 100\%. \\
    \\
    \textit{Average years of education.} In contrast to the aformentioned statistics, the variation in average years of educational attainment is smaller, suggesting a tighter distribution (1.354). Most observations have statistics close to the sample mean (7.031, the range beign 2.488 to 11.870). \\
    \\
    \textit{Local hotel sales.} Local hotel sales vary massively from the mean of 27,120.140 and a standard deviation of 201,033.400. Many municipalities record neglible sales (minimum is 1). The distribution appears to be right skewed with a long tail towards higher sales (maximum is 5,229,616). \\
    \\
    \textit{Logarithm of temperatures and logarithm of precipitation.} The logarithm of temperatures across municipalities ranges from a low of 4.658 to a maximum of 5.672. The mean log temperature is 5.283. For precipation, the range is from a minimum of 1.790 to a maximum of 5.746, and the mean is 4.304.
    \\ \\
    \textit{Kilometer distance to the US border.} The distance to the US border ranges from a minimum value of 6.609 and a maximum of 1384.003. The mean distance is 696.171 with a standard devation of 278.301.
    \pagebreak
    ...
    The difference between the OLS and IV point estimates for $\beta_1$ was different than what I had expected in part C) as I identified two reasons why the OLS estimate might be the result of positive bias (reverse causality and omitted variable bias) and one where negative bias might be at play (measurement error). However, the new IV point estimate suggests there was negative bias deflating the true value of $\beta_1$, the effect of an extra year of average years of education on local average monthly incomes. 
    ...
    It's possible that our instrument also does not pass the instrument exclusion restriction. While the effect of the logarithm of distance to the US border might not be statistically significant, its effect on the logarithm of local average monthly household incomes is. In this case, our instrument does not have an affect on the dependent variable only through our endogenous explanatory variable but rather through other ways.
        ...
        \] 
    \\
    For a potential instrumental variable to be a valid candidate, the effect it has on the dependent variable must only act through the explanatory one (through `a' and then `b' in the diagram above). If it impacts $Y$ through `c' for instance, it is not valid as the explanatory variable is no longer exogenous. In regression (4) we can see how, in contrast to (2), the point estimate for the causal effect of $ln(dist\_us\_km)$ on $ln(inc\_m)$ is statistically significant at the 0.01 level. That is, our point estimate for $\gamma_1$ is statistically significant. This suggests that the distance to the US border does have a causal effect on local average monthly household incomes, but not through average years of education. An intuitive reason for why this might be the case is because proximity to the United States border, and therefore its larger markets, could have an effect on local average monthly household incomes for these municipalities.
